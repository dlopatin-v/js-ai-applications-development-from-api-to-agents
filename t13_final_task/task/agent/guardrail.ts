...
export class UMSDataGuardrail {
  ...
  redact(text: string): string {
    // TODO:
    // 1. Iterate `CC_PATTERN_SOURCES`; for each `[src, flags]`, replace matches in `text` with `REDACTED`
    //    using `text.replace(new RegExp(src, flags), REDACTED)`
    // 2. Repeat the same loop for `SALARY_PATTERN_SOURCES`
