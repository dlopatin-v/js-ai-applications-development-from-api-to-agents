...
- `scoreThreshold`: Similarity threshold (range: 0.1-0.99, default: 0.5)
...
### **Step 2: Embeddings Client [embeddings_client.ts](embeddings/embeddings_client.ts)**
- Complete the `getEmbeddings()` method to call embeddings API
- Parse the response and extract embeddings data
- Handle the request/response format according to API specification

### **Step 3: Text Processing [text_processor.ts](embeddings/text_processor.ts)**
...
### **Step 4: Main Application [app.ts](app.ts)**
...
2. In `app.ts`, find the `textProcessor.search(...)` call and change `dimensions: 384` to `dimensions: 385`
...
#### Step 4: Switch `embeddingsClient` to Ollama in `app.ts`
