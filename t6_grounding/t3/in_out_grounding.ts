...
//    'cold start' add all users for current moment to vectorstore and with each user request we will update vectorstore on
//    the retrieval step, we will remove deleted users and add new - it will also resolve the issue with consistency
//    within this 2 services and will reduce costs (we don't need on each user request load vectorstore from scratch and pay for it).
